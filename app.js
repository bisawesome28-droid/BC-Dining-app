...
function renderRowPeriods(r) {
  ...
  return `
    <div class="row-periods">
      ${r.periods.map((p) => `
        <div class="row-period" style="background:${p.cur ? 'rgba(86,2,10,.22)' : 'transparent'}">
...
function buildScheduleSection(schedule, day, dateStr) {
  ...
  const periods = ps.map((pd) => {
    ...
    return {
      ...
      dot: cur ? '#34c759' : done ? 'rgba(36,33,36,.28)' : 'rgba(86,2,10,.85)'
    };
  });
...
function renderScheduleBlock(title, section, day) {
  ...
  return `
    ...
    <div class="posted-week">
      ${section.week.map((w) => `
        <div class="posted-week-row" style="background:${w.isSel ? 'rgba(86,2,10,.16)' : 'transparent'};border-left-color:${w.isSel ? '#56020a' : 'transparent'}">
